...
pub(super) enum RunCommand {
    ...
    #[command(
        long_about = "Start one persistent Run. By default the CLI waits for the Engine to return: stderr emits an NDJSON Live Event stream and stdout receives one compact JSON summary. Use run get for the complete persisted Run record. --detach starts the same Coordinator in an independent process group, waits until Run acceptance is visible, then returns the Run ID and recovery command without streaming Program output; use run get or query afterwards. The selected Catalog name, description, and labels are immutable caller facts and are not execution facts. Reusing a Run identity requires the same semantic input and metadata.",
