...
use chrono::Utc;
use run_engine::{EngineEventSink, EngineStage, ProgramStream};
use run_protocol::ProgramId;
use serde_json::{Value, json};

const LIVE_EVENT_QUEUE_CAPACITY: usize = 256;

pub(crate) struct RunLiveEvent {
    inner: Arc<LiveEventInner>,
...
impl RunLiveEvent {
    ...
    fn with_writer(run_id: Option<&str>, writer: impl Write + Send + 'static) -> Self {
        let (sender, receiver) = sync_channel(LIVE_EVENT_QUEUE_CAPACITY);
        let inner = Arc::new(LiveEventInner {
            sender,
            enabled: true,
            dropped_event: AtomicBool::new(false),
    ...
    pub(crate) fn engine_event_sink(&self) -> Arc<dyn EngineEventSink> {
        Arc::clone(&self.inner) as Arc<dyn EngineEventSink>
    }
    ...
    pub(crate) fn report_dropped_events(&self) {
        if self.inner.dropped_event.swap(false, Ordering::AcqRel) {
            self.send(json!({
                "kind": "transport.diagnostic",
                "observed_at": observed_at(),
                "operation": "stderr_live_event",
                "message": "Live Event records were dropped because the stderr consumer could not keep up",
            }));
        }
    }
    ...
    fn enqueue(&self, value: Value) {
        ...
        match self.sender.try_send(value) {
            ...
            Err(TrySendError::Full(_)) => {
                self.dropped_event.store(true, Ordering::Release);
            }
        }
    }
}

impl EngineEventSink for LiveEventInner {
