...
- `run_engine` 拥有 `RunEngine`、调用级取消、OCI 内容访问和具体 Engine 实现。
- `runlab` 拥有 CLI、请求构造、Catalog、Run identity、Run Record、Storage、Coordinator 和 VM transport。

当前阶段只完成前两个 library package，并且只实现 NativeEngine。DockerEngine 已明确推迟；`runlab` 的产品接线与旧执行路径删除也属于后续阶段，不在协议或 Engine API 中预留兼容模型。
...
Exit gate：Native 在真实 Linux OCI Runtime 验证，并完成独立 review。

## Deferred：DockerEngine 与跨 Engine conformance

DockerEngine 不属于当前分支。未来只有在重新进入该实施阶段后，才定义它可忠实映射的 OCI 子集并运行跨 Engine conformance。
...
只有两个真实实现都跑通后，才允许提取已经证明重复的私有机制；不增加公共 Backend trait。

Exit gate：全 workspace 门禁、真实环境测试、跨 Engine conformance 和最终独立 review 全部通过。

## 提交原则

checkpoint 之后恢复小而完整的 feature 提交。每个提交必须自行编译并携带对应测试。公共 API 与 Cargo workspace 文件由一个 integrator 持有；并行任务只能修改预先分配的互斥文件。
